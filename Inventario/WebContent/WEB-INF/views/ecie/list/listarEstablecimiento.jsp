	...
	view: function(){
		...
		var ds = new Ext.data.JsonStore({
	   	...
	   	var gp = new Ext.grid.GridPanel({
			id: 'establecimientoGrid',
			title: 'Listado de los Establecimientos',
			border: false,
			height: 500,
        	store: ds,
			...
			tbar: new Ext.Toolbar({
				items: [{
					...
					tooltip:'Adicionar Establecimiento',
					listeners:{
						click:function(){editarEstablecimiento('../${modulo}/editarEstablecimiento.htm');}		
						}			  			
					...
					listeners:{
						click:function(){
							if(gp.selModel.hasSelection()){
								modificar(gp, '../${modulo}/editarEstablecimiento.htm');
            					...
            					Ext.MessageBox.show({  
									...
									msg: 'Debe seleccionar un Establecimiento.',  
				...
				
				{
					...
					tooltip:'Eliminar Establecimiento',
					listeners:{
						click:function(){
							if(gp.selModel.hasSelection()){
								Ext.MessageBox.show({
				    		       title:'Confirmación',
						           msg: '¿Desea eliminar el Establecimiento?',
						           buttons: Ext.MessageBox.YESNO,
						           fn: function (btn){
								       	 if (btn == 'yes'){
								       	 	Ext.Ajax.request({
												url: '../${modulo}/eliminarEstablecimiento.htm',
												params: {
				...
				{xtype: 'tbspacer'},
				{
					...
					tooltip:'Imprimir Establecimientos',
					listeners:{
						click:function(){
							descargar('../ecie/imprimirEstablecimiento.htm', ds.baseParams);
				  		}	
			  		}
				}]
			}),
		...
		function editarEstablecimiento(url, id){
			cargarVentana(url, {id: id});
	    }
   var tp = new Ext.TabPanel({
			id: 'establecimientoTabPanel',
			border: false,
	        monitorResize:true,
		...
		var establecimientoPanel = new Ext.Panel({
		  border: false,
		  layout: 'fit',
		  title:'Nomenclador',
		  items:[tp] 
