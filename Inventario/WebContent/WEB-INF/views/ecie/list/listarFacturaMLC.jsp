...
<aek:jsmin>
{
	...
	'</span>';
		...
		var gp = new Ext.grid.GridPanel({
			...
			tbar: new Ext.Toolbar({
				items: [{
					...
					listeners:{
						click:function(){editarSolicitudEntrega('../${modulo}/editarFacturaMLC.htm');}			  			
			  		}
				},{
					text: 'Detalles',
					cls:'x-btn-text-icon bmenu',
					...
					listeners:{
						click:function(){
							if(gp.selModel.hasSelection()){
								detalles(gp, tp, '../${modulo}/detallesFacturaMLC.htm');
							}
							else{
	           					Ext.MessageBox.show({  
					...
					listeners:{
						click:function(){
							if(gp.selModel.hasSelection()){
								modificar(gp, '../${modulo}/editarFacturaMLC.htm');
		...
		function editarSolicitudEntrega(url, id){
			cargarVentana(url, {id: id, tipo: 'FacturaMLC'});
	    }
