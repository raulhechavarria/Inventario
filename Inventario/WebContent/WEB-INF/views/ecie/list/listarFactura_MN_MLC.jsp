...
<aek:jsmin>
{
	...
	view: function(){
		...
		var ds = new Ext.data.JsonStore({
			...
			baseParams: {'tipo': 'Factura_MN_MLC' },
 			autoLoad: true,
			root: 'data',
	...
	'</span>';
		...
		var gp = new Ext.grid.GridPanel({
			...
			tbar: new Ext.Toolbar({
				items: [{
					...
					listeners:{
						click:function(){editarSolicitudEntrega('../${modulo}/editarFactura_MN_MLC.htm');}			  			
			  		}
				},{
					text: 'Detalles',
					cls:'x-btn-text-icon bmenu',
					...
					listeners:{
						click:function(){
							if(gp.selModel.hasSelection()){
								detalles(gp, tp, '../${modulo}/detallesFactura_MN_MLC.htm');
							}
							else{
	           					Ext.MessageBox.show({  
					...
					listeners:{
						click:function(){
							if(gp.selModel.hasSelection()){
								modificar(gp, '../${modulo}/editarFactura_MN_MLC.htm');
		...
		function editarSolicitudEntrega(url, id){
			cargarVentana(url, {id: id, tipo: 'Factura_MN_MLC'});
	    }
