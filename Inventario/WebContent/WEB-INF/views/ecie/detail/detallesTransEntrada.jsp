...
<aek:jsmin>
{
	...
	view: function(){
		...
		var detallesTransEntradaGrid = new Ext.grid.GridPanel({
			id: 'detallesTransEntradaGrid',
			loadMask:true,
			title:'Productos',
			monitorResize:true,
			border: false,
			...
			tbar: new Ext.Toolbar({
				items: [
				{
					...
					listeners:{
						click:function(){
							if(detallesTransEntradaGrid.selModel.hasSelection()){
							
								modificarVale(detallesTransEntradaGrid, '../${modulo}/editarEntradaValeProducto.htm',{ 'idVale': '${command.id}'});
						}
							else{
            					Ext.MessageBox.show({  
									title: 'Productos',  
								...
								Ext.MessageBox.show({
						           ...
						           fn: function (btn){
								       	 if (btn == 'yes'){
												...
												params: {
													id: detallesTransEntradaGrid.selModel.getSelected().id
												},
												success: function (response, options) {
													if(Ext.decode(response.responseText).success == true){
														detallesTransEntradaGrid.getStore().reload();
													}
													else{
		...
		var detallesTransEntrada= new Ext.Panel({
			id: 'detallesTransEntrada',
			title: 'Detalles Transferencia Entrada',
			closable: true,
			layout: 'border',
			height: 600,
			border:false,
			...
			items:[
				new Ext.form.FormPanel({
					...
					{xtype: 'tbspacer'},
					{
						...
						tooltip:'Imprimir Detalles de Transferencia Entrada',
							listeners:{
								click:function(){
									descargar('../ecie/impDetallesTransEntrada.htm', { idVale: '${command.id}','tipo': 'TransferenciaEntrada'});
							}			  			
				  		}
					}
	   ...
	   detallesTransEntradaGrid.on('celldblclick', gridCellDblClick, this);
		
		function gridCellDblClick(target, rowIndex, columnIndex, e){
			cargarVentana('../${modulo}/editarEntradaValeProducto.htm', {id: detallesTransEntradaGrid.selModel.getSelected().id,'idVale': '${command.id}'});
		}
	   ...
	   function modificarVale(detallesTransEntradaGrid, url, id){
	   var idm = id.idVale;
			cargarVentana(url, {id: detallesTransEntradaGrid.selModel.getSelected().id,idVale:idm});
		}
	   
	   
		return detallesTransEntrada;			
	}
}
</aek:jsmin>