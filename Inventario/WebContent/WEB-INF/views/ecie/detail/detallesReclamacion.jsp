...
<aek:jsmin>
{
	...
	view: function(){
		...
		var recepcionStore = new Ext.data.JsonStore({
            ...
            autoLoad: true,
       	    remoteSort: true,
			baseParams: {limit:20, 'tipo': 'Reclamacion' },
  	        root: 'data',
    	    totalProperty: 'totalCount',
    	    id: 'id',
		...
		var leftPanel = new Ext.Panel({
			...
			items:[
				...
				new Ext.form.DisplayField({
					value:'${command.recargoMN}',
					fieldLabel:'Recargo CUP'
				}),
				new Ext.form.DisplayField({
					value:'${command.recargoMLC}',
					fieldLabel:'Recargo CUC'
				}),
				new Ext.form.DisplayField({
		...
		var centerPanel = new Ext.Panel({
			...
			items:[
				...
				new Ext.form.DisplayField({
					value:'${command.descuentoMN}',
					fieldLabel:'Descuento CUP'
				}),
				new Ext.form.DisplayField({
					value:'${command.descuentoMLC}',
					fieldLabel:'Descuento CUC'
				}),
				new Ext.form.DisplayField({
		...
		var detallesReclamacionGrid = new Ext.grid.GridPanel({
			id: 'detallesReclamacionGrid',
			loadMask:true,
			title:'Productos',
			monitorResize:true,
			border: false,
			...
			tbar: new Ext.Toolbar({
				items: [
				{
					...
					listeners:{
						click:function(){
							if(detallesReclamacionGrid.selModel.hasSelection()){
							
								modificarVale(detallesReclamacionGrid, '../${modulo}/editarEntradaValeProducto.htm',{ 'idVale': '${command.id}'});
						}
							else{
            					Ext.MessageBox.show({  
									title: 'Productos',  
								...
								Ext.MessageBox.show({
						           ...
						           fn: function (btn){
								       	 if (btn == 'yes'){
												...
												params: {
													id: detallesReclamacionGrid.selModel.getSelected().id
												},
												success: function (response, options) {
													if(Ext.decode(response.responseText).success == true){
														detallesReclamacionGrid.getStore().reload();
													}
													else{
		...
		var detallesReclamacion= new Ext.Panel({
			id: 'detallesReclamacion',
			title: 'Detalles Reclamacion',
			closable: true,
			layout: 'border',
			height: 600,
			border:false,
			...
			items:[
				new Ext.form.FormPanel({
					...
					{xtype: 'tbspacer'},
					{
						...
						tooltip:'Imprimir Detalles del Reclamacion',
							listeners:{
								click:function(){
									descargar('../ecie/impDetallesReclamacion.htm', { idVale: '${command.id}','tipo': 'Reclamacion'});
							}			  			
				  		}
					}
				...
				new Ext.TabPanel({
					id:'reclamacionTabPanel',
					region:'center',
					border: false,
					activeTab:0,
					items:[
						detallesReclamacionGrid
					]
				})
	   ...
	   detallesReclamacionGrid.on('celldblclick', gridCellDblClick, this);
		
		function gridCellDblClick(target, rowIndex, columnIndex, e){
			cargarVentana('../${modulo}/editarSalidaValeProducto.htm', {id: detallesReclamacionGrid.selModel.getSelected().id,'idVale': '${command.id}'});
		}
	   ...
	   function modificarVale(detallesReclamacionGrid, url, id){
	   var idm = id.idVale;
			cargarVentana(url, {id: detallesReclamacionGrid.selModel.getSelected().id,idVale:idm});
		}
